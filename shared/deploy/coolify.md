...
This guide deploys ONE impl per stack. Both Go and TS are wire-compatible; pick one. See [`README.md`](README.md) for the comparison.
...
* Set **Base Directory** to `/shared/deploy`
* Set **Docker Compose Location** to `/shared/deploy/docker-compose.yml`
...
Use your actual hostname for `SEMP_HOST`. `SEMP_IMPL` is `go` or `ts`.
...
Coolify automatically attaches the service to its proxy network, so no `networks:` block is needed. The `${SEMP_HOST:?...}` syntax fails the deploy with a clear error if the env var is unset; `${SEMP_IMPL:-go}` falls back to Go.

### 4. Configure semp.toml

Edit `shared/deploy/semp.toml` in the repo (or use Coolify's **Mounts** tab to provide a file at `/etc/semp/semp.toml`):
...
Change `SEMP_IMPL` in **Environment Variables** and redeploy. The container is rebuilt from the other Dockerfile against the same `semp-data` volume; the SQL schema is identical between impls so existing data persists.

For interop testing, clone the resource into a second deployment with a different host label and the other impl, then federate them via `[[federation.peers]]`.
